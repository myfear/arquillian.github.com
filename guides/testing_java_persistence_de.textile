---
layout: guide
title: Java Persistence Testen
authors: [mojavelinux, bartoszmajsak, hasalex]
translators: [myfear]
tags: [jpa, database, persistence, transactions]
description: Teste Deine Daten! Lerne, wie Java Persistence (JPA) queries mit mehreren Providern in einem Arquillian Test getestet werden. 
guide_group: 2
guide_order: 1
TODO: ["use utility EJB to handle transaction boundaries?"]
---
Dieser Leitfaden zeigt Dir, wie Du Arquillian zum Testen Deiner Java Persistence (JPA) Schicht verwendest. Nach dem Lesen kennst Du die folgenden Dinge:

* Einen Test erstellen, welcher den JPA deployment descriptor (persistence.xml) enthält
* Den EntityManager und die UserTransaction in einen Test injizieren
* Entitäten persistieren und via JPQL und JPA2 Criteria API wieder laden
* Tests unter Verwendung verschiedener JPA Provider ausführen

Schnell wirst Du sehen, dass Arquillian das perfekte Rezept zum Testen von JPA liefert. Dieser Leitfaden ist geschrieben um ihm einfach folgen zu können und jederzeit zurückzukommen, wann auch immer Du mit JPA kodieren musst.

h3. Annahmen

Wir gehen davona us, dass Du entweder den "Erste Schritte":/guides/getting_started_de oder den "Noch schneller Starten mit Forge":/guides/get_started_faster_with_forge_de Leitfaden gelesen hast und bereits ein Arquillian Test Suite Setup in einem Maven Projekt vorliegen hast. Lösche gerne die bereits vorhandenen Klassen, um Plat für die neuen zu machen. Wir fügen eine JPA Entity im Projekt hinzu um einen einfachen Java Persistence Test zu erstellen. Von dort aus kannst Du die Anweisungen auf jede andere Entität, welche Du testen möchtest übertragen.

Die Anweisungen in diesem Leitfaden sind spezielle für ein Maven basiertes Projekt ausgelegt, auch wenn Arquillian nicht an Maven gebunden ist. Die Tests werden im Embeddedd GlassFish und im lokalen JBoss AS 7 ausgeführt. Du kannst aber auch jeden anderen Container mit einem verfügbaren JPA Provider verwenden, der von Arquillian unterstützt wird.

p(warning). %Das arquillian-weld-ee-embedded Profil kann in diesem Leitfaden nicht verwendet werden, da Weld keinen JPA Service bereitstellt. (Weld stellt nur CDI Funktionalitäten zur Verfügung).%

h3. Zweck

Die Anwendung enthält eine (Video-) @Game@ Entität mit zwei Attributen:

* @id@ - Der Primärschlüssel
* @title@ - Der Titel des Spiels

Wir werden einen Test schreiben, welcher Beispiel-Entitäten in die Datenbank persistiert und diese dann sowohl mit JPQL als auch mit der JPA2 Criteria API wieder läd. Am Ende wird der Test die folgenden drei Schritte ausführen:

* Beispiel-Entitäten mit dem JPA @EntityManager@ in die Datenbank schreiben 
* Die Datenbank mithilfe von JPQL abfragen
* Die Datenbank mithilfe der JPA 2 Criteria API abfragen

Der komplette Source-Code ist im "Arquillian examples project":http://github.com/arquillian/arquillian-examples/tree/master/arquillian-persistence-tutorial auf github verfügbar. Um ihn in Aktion zu sehen musst Du lediglich das folgende Kommando eingeben (und ein wenig Geduld für Maven haben, bis alle abhängigen Bibliotheken heruntergeladen sind).

bc(command). mvn test

Nun schauen wir uns an, wie das genau funktioniert.

h3. Project Struktur

Um Dich einzugewöhnen schauen wir uns die Verzeichnisstruktur des Projekts an:
...
@Game@ ist die JPA Entity Klasse und test-persistence.xml ist eine modofizierte Version der persistence.xml die eine Definition der Persistence Unit für die Test Umgebung enthält. Beachte, dass zwei Test Ordner eine test-persistence.xml enthalten. Eine für jeden Container welchen wir verwenden werden. Dazu mehr später.

Wir empfehlen die Verwendung eines separaten JPA Deskriptors für jeden zu testenden Container als optimale Vorgehensweise um den Test von der Produktion unterschiedlich zu Konfigurieren. Beispielsweise möchte man in der Testumgebung eine "drop-and-create-tables" Strategie verwenden um die Schemata der DataSource zu verwalten. Auch ist das Loggen der Queries in einer Testumgebung durchaus gewünscht. Beides würde in Produktivumgebungen nicht sinnvoll sein. Mit den verschiedenen test-persistence.xml Deployment Deskriptoren kann genau dies erfolgen, ohne die eigentliche Anwendung zu beeinflussen. Die eigentliche persistence.xml wird unverändert gelassen, nachdem diese lediglich die Definitionen für die Produktivumgebung enthält.

Hier ist der Source-Code der @Game@ Entity Klasse:
...
p. Der Primärschlüssel wird mit der @@Id@ Annotation am gleichnamigen Attribut gesetzt. Weitere Spalten werden automatisch aus den Bean Properties (standard getter/setter convention) ermittelt. Darüber hinaus können diese aber auch mit der @@Column@ Annotation explizit gesetzt werden. Ohne Annotation gilt die Konvention, dass der Spaltenname in der Datenbank aus dem Attribut-Namen oder dem Namen der Eigenschaft (analog der JavaBean Konvention) ohne das "get" Prefix und mit vorangestelltem Kleinbuchstaben gebildet wird (bspw., getTitle() &rarr; title).  

Darüber hinaus verwenden wir die Standard Annotationen der Bean Validation um Werteinschränkungen zu prüfen. So muss ein Titel enthalten sein (@@NotNull@) und er muss zwischen 3 und 50 Zeichen lang sein. (Hinweis: Das würde auch einen guten Test hergeben).

h3. Den Test Schreiben

Wo wir grade bei Tests sind erstellen wir direkt einen neuen JUnit 4 Arquillian Testfall mit dem Namen @GamePersistenceTest@ und bereiten wir diesen für die JPA Operationen vor. Dabei verwenden wir die Möglichkeiten von "CDI (JSR-299)":http://docs.jboss.org/cdi/spec/1.0/html um die benötigen Ressourcen per Dependency Injection zu erreichen. (Alternativ könnten die Transaktionsgrenzen auch von einer EJB übernommen werden; das erfolgt aber in einem späteren Leitfaden.)
